public class BSTree<T extends Comparable<? super T>> {   
    ...
    // ED208 = Método para encontrar o menor valor na árvore
    public T minValue() {
        if (isEmpty()) {
    ...
    // ED208 - Método para encontrar o maior valor na árvore
    public T maxValue() {
        if (isEmpty()) {
            throw new IllegalStateException("A árvore está vazia.");
        }
    ...
    // ED209 -  Método para contar os elementos no intervalo [a, b]
    public int countBetween(T a, T b) {
        if (isEmpty()) {
            return 0; // Se a árvore estiver vazia, nenhum elemento está no intervalo
        }
    ...
    private int countBetween(BSTNode<T> node, T a, T b) {
        ...
        // Se o valor do nó estiver no intervalo [a, b], conta o nó
        if (node.getValue().compareTo(a) >= 0 && node.getValue().compareTo(b) <= 0) {
            count = 1;
        }

        // Explorar a subárvore esquerda apenas se houver chance de encontrar valores ≥ a
        if (node.getValue().compareTo(a) > 0) {
            count += countBetween(node.getLeft(), a, b);
        }

        // Explorar a subárvore direita apenas se houver chance de encontrar valores ≤ b
        if (node.getValue().compareTo(b) < 0) {
            count += countBetween(node.getRight(), a, b);
        }

        return count;
    }

    // ED210 - Método para verificar se a árvore é uma árvore binária de pesquisa (BST)
    ...
    private boolean valid(BSTNode<T> node, T min, T max) {
        if (node == null) {
            return true; // Caso base: uma subárvore vazia é válida
        }

        T value = node.getValue();

        // Verificar se o valor do nó está fora dos limites permitidos
        if ((min != null && value.compareTo(min) <= 0) || (max != null && value.compareTo(max) >= 0)) {
            return false;
        }

        // Recursivamente verificar subárvores esquerda e direita
        return valid(node.getLeft(), min, value) && valid(node.getRight(), value, max);
    }
